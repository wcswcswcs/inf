...
USE_GEO=1                         # 1=跑 geo prune budgets；0=不跑
BASELINE_BUDGET=1200000           # baseline 的 budget
...
run_job() {
  ...
  if [[ "$kind" == "geo" ]]; then
    ...
    out_path="${OUT_DIR}/${scene_id}_baseline.pth"
    log_path="${LOG_DIR}/${scene_id}_baseline.log"
  fi
  ...
  tag="GPU${gpu}|${scene_id}|${kind}|${budget}"
  prefix="[$tag] "
  ...
  log_line "$prefix" "$log_path" "Kind: $kind"
  ...
  # 关键点：
  # - python -u / PYTHONUNBUFFERED=1 保证 python 不缓冲
  # - 用 bash while read 逐行加前缀，最大兼容
  # - tee 同时写 log 和终端
  if [[ "$kind" == "geo" ]]; then
  ...
  # 取 python 的退出码（pipeline 的第 1 个命令）
  local py_rc tee_rc
  py_rc="${PIPESTATUS[0]}"
  tee_rc="${PIPESTATUS[2]}"
...
# ===== 生成任务列表 =====
declare -a JOB_SCENE_PATH JOB_SCENE_NAME JOB_KIND JOB_BUDGET

for scene_path in "${SCENE_DIRS[@]}"; do
  [[ -d "$scene_path" ]] || continue
  scene_name="$(basename "${scene_path%/}")"

  if [[ "$USE_GEO" -eq 1 ]]; then
    for b in "${BUDGETS[@]}"; do
...
while true; do
  ...
  for ((i=0; i<num_gpus; i++)); do
    ...
    if [[ "$pid" -ne 0 ]] && ! is_alive "$pid"; then
      ...
      if [[ "$rc" -ne 0 ]]; then
        ...
        kind="${JOB_KIND[$ended_idx]}"
        bud="${JOB_BUDGET[$ended_idx]}"
        echo "[SCHED] FAIL rc=$rc | job=$((ended_idx+1)) | gpu=$g | $scene | $kind | $bud"
        echo "job=$((ended_idx+1)) gpu=$g rc=$rc scene=\"$scene\" kind=$kind budget=$bud" >> "$FAIL_FILE"
      else
  ...
  for ((i=0; i<num_gpus; i++)); do
    ...
    if [[ "${GPU_PID[i]}" -eq 0 && "$job_idx" -lt "$NUM_JOBS" ]]; then
      ...
      kind="${JOB_KIND[$job_idx]}"
      bud="${JOB_BUDGET[$job_idx]}"

      echo "[SCHED] START job $((job_idx+1))/$NUM_JOBS | gpu=$g | $scene_name | $kind | $bud"

      run_job "$g" "$scene_path" "$scene_name" "$kind" "$bud" &
      GPU_PID[i]=$!
