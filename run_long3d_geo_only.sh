...
# ===== 可选参数：只跑某个 scene =====
# 例如：./run_long3d_geo_only.sh "Badminton Court"
SCENE_FILTER="${1:-}"
...
if [[ "${#JOB_BUDGET[@]}" -eq 0 ]]; then
  echo "ERROR: no jobs created."
  if [[ -n "$SCENE_FILTER" ]]; then
    echo "       scene '$SCENE_FILTER' not found under $DATA_ROOT (case/space sensitive)."
  fi
  exit 1
fi
