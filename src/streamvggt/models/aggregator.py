...
class Aggregator(nn.Module):
    ...
    def __init__(
        ...
        geo_selector_impl: str = "legacy_v1",
        geo_v2_enable_reference: bool = False,
        geo_v2_debug_log: bool = True,
        geo_v2_anchor_budget_ratio: float = 0.28,
        geo_v2_stable_budget_ratio: float = 0.20,
        geo_v2_keyframe_budget_ratio: float = 0.10,
        geo_v2_recent_budget_ratio: float = 0.12,
        geo_v2_recent_budget_cap: int = 2048,
        geo_v2_recent_per_frame_cap: int = 96,
        geo_v2_frame0_quota: int = 1024,
        geo_v2_keyframe_frozen_quota: int = 256,
        geo_v2_phase_good_streak_thr: int = 8,
        geo_v2_map_stable_streak_thr: int = 16,
    ):
        super().__init__()
        ...
        self.geo_selector_impl = str(geo_selector_impl)
        self.geo_v2_enable_reference = bool(geo_v2_enable_reference)
        self.geo_v2_debug_log = bool(geo_v2_debug_log)
        self.geo_v2_anchor_budget_ratio = float(min(max(geo_v2_anchor_budget_ratio, 0.0), 1.0))
        self.geo_v2_stable_budget_ratio = float(min(max(geo_v2_stable_budget_ratio, 0.0), 1.0))
        self.geo_v2_keyframe_budget_ratio = float(min(max(geo_v2_keyframe_budget_ratio, 0.0), 1.0))
        self.geo_v2_recent_budget_ratio = float(min(max(geo_v2_recent_budget_ratio, 0.0), 1.0))
        self.geo_v2_recent_budget_cap = int(max(128, geo_v2_recent_budget_cap))
        self.geo_v2_recent_per_frame_cap = int(max(8, geo_v2_recent_per_frame_cap))
        self.geo_v2_frame0_quota = int(max(256, geo_v2_frame0_quota))
        self.geo_v2_keyframe_frozen_quota = int(max(64, geo_v2_keyframe_frozen_quota))
        self.geo_v2_phase_good_streak_thr = int(max(1, geo_v2_phase_good_streak_thr))
        self.geo_v2_map_stable_streak_thr = int(max(1, geo_v2_map_stable_streak_thr))
    ...
    def reset_geo_cache_state(self):
        ...
        self.geo_frozen_reference_refresh_count: int = 0
        self.geo_frozen_reference_refresh_reason: str = "none"
    ...
    def _geo_v2_reset_runtime(self) -> None:
        ...
        self.geo_v2_last_metrics: Dict[str, Any] = {}
        self.geo_v2_last_keep_idx = torch.empty((0,), dtype=torch.long)
        self.geo_v2_anchor_visible_coverage_ema = 0.0
        self.geo_v2_stable_visible_coverage_ema = 0.0
        self.geo_v2_recent_plain_ratio_ema = 0.0
        self.geo_v2_long_horizon_keep_ratio_ema = 0.0
        self.geo_v2_reanchor_added_ema = 0.0
    ...
    def export_geo_cache_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "geo_frozen_reference_refresh_count": int(self.geo_frozen_reference_refresh_count),
            "geo_frozen_reference_refresh_reason": str(self.geo_frozen_reference_refresh_reason),
    ...
    def load_geo_cache_state(self, state: Dict[str, Any]) -> None:
        ...
        self.geo_frozen_reference_refresh_count = int(state.get("geo_frozen_reference_refresh_count", 0))
        self.geo_frozen_reference_refresh_reason = str(state.get("geo_frozen_reference_refresh_reason", "none"))
    ...
    def _maybe_freeze_reference_bank(self, frame_idx: int) -> None:
        ...
        base_freeze_ready = bool(
            ...
            and int(self.geo_current_release_streak) >= 3
        ...
        if frozen:
            ...
            if rescue_refresh_due:
                ...
                self.geo_frozen_reference_refresh_reason = "initial_freeze"
    ...
    def _geo_preview_adaptive_policy(self, *, frame_idx: int, metrics: Dict[str, float]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        instability_preview_old = float(metrics["instability"])
    ...
    def _geo_commit_adaptive_policy_once(
        ...
        *,
        ...
        self.geo_instability_ema = float(policy.get("instability_committed", metrics["instability"]))
        ...
        self.geo_last_policy_inputs = {
            ...
            "instability_preview_old": float(policy.get("instability_preview_old", metrics["instability"])),
            "instability_preview_new": float(policy.get("instability_preview_new", self.geo_instability_ema)),
            "instability_committed": float(policy.get("instability_committed", self.geo_instability_ema)),
        }
    ...
    def update_geo_frame_metadata(
        ...
        self.geo_last_policy_inputs["geo_frozen_reference_refresh_count"] = int(self.geo_frozen_reference_refresh_count)
        self.geo_last_policy_inputs["geo_frozen_reference_refresh_reason"] = str(self.geo_frozen_reference_refresh_reason)
    ...
    def _emit_console_geo_log(
        ...
        print(
            ...
            f"anchor_in_cache={int(inp.get('anchor_in_cache', 0) or 0)} "
            f"ref_bank={int(inp.get('ref_bank_size', len(self.geo_reference_bank)) or 0)} "
    ...
    def _finalize_geo_keep(
        ...
        *,
        ...
        mode_feedback = str((policy or {}).get("mode", "legacy"))
        ...
        self.geo_last_keep_feedback = {
            ...
            "reloc_feedback_updated": bool(reloc_feedback_updated),
    ...
    def _geo_reloc_release_ready(self) -> bool:
        ...
        selector_diag_fresh = bool(
            ...
            and int(selector_diag.get("frame_idx", -1)) >= int(self.geo_last_policy_frame) - 2
        )
        return bool(
            self._geo_structure_ready()
        ...
        *,
        ...
        if frame0_idx.numel() > 0:
            if mode_now == "legacy" or max_past_tokens is None:
                ...
                frame0_keep = frame0_idx
            elif not structure_mature:
                frame0_hard_scale = max(float((policy or {}).get("frame0_hard_scale", 1.0)), 0.85)
                base_frame0_quota = min(
                    int(frame0_idx.numel()),
                    max(768, int(0.18 * int(max_past_tokens))),
                )
                frame0_quota = min(
                    int(frame0_idx.numel()),
    ...
    def _select_geo_active_indices(
        ...
        if structure_ready and self.geo_selection_interval > 1 and (not force_full_select) and (current_frame_idx % self.geo_selection_interval != 0):
            if mode_now in {"current", "recovery"}:
                return self._select_geo_active_indices_current_micro(
                    meta=meta,
                    selected=selected,
                    selected_order=selected_order,
                    current_frame_idx=current_frame_idx,
        ...
        if mode_now in {"current", "recovery", "reloc"} and max_past_tokens is not None:
            recent_plain_idx = torch.nonzero(
                recent_mask
                & (~is_special)
                & (geo_role == 0)
            ...
            if recent_plain_idx.numel() > 0:
                ...
                if mode_now == "reloc":
                    recent_plain_quota = max(int(recent_plain_quota), 192)
                recent_plain_frame = frame_idx.index_select(0, recent_plain_idx)
                uniq_recent_frames = torch.unique(recent_plain_frame).sort(descending=True).values
                per_frame_quota = max(
                    16,
    ...
    def _select_geo_active_indices_current_micro(
        self,
        *,
        meta: Dict[str, torch.Tensor],
        selected: set[int],
    ...
    def forward(
        ...
        geo_selector_impl_override: Optional[str] = None,
        ...
        if use_cache and use_geo_kv_prune and any(kv is not None for kv in past_key_values):
            ...
            self.geo_last_policy_inputs["prestructure_ref_ready"] = bool((geo_policy or {}).get("prestructure_ref_ready", False))
            ...
            if ref_meta is not None:
                ...
                if mode_now == "legacy":
                    self.geo_last_policy_inputs["recovery_selector"] = bool(False)
                    geo_reloc_active = False
                    if str(selector_impl_eff) == "map_first_v2":
                        geo_shared_keep_idx = self._select_geo_active_indices_map_first_v2(
                            meta=ref_meta,
                ...
                if attn_type == "frame":
                    ...
                    if use_cache:
                        ...
                        if use_geo_kv_prune:
                            ...
                            anchor_enabled = bool((geo_policy or {}).get("use_anchor_labels", False)) and (not v2_legacy_active)
                            landmark_enabled = bool((geo_policy or {}).get("use_landmark_labels", False)) and (not v2_legacy_active)
                            reference_enabled = bool((geo_policy or {}).get("use_reference_labels", False)) and (not v2_legacy_active)
