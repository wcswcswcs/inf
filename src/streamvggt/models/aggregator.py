...
class Aggregator(nn.Module):
    ...
    def _geo_preview_adaptive_policy(self, *, frame_idx: int, metrics: Dict[str, float]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        immature_ref_structure = bool((not structure_ready) and len(self.geo_reference_bank) < 32)
        if immature_ref_structure:
            policy["frame0_hard_scale"] = max(float(policy.get("frame0_hard_scale", 1.0)), 0.75)
            policy["reference_hard_scale"] = max(float(policy.get("reference_hard_scale", 1.0)), 0.80)
        if structure_ready and float(metrics["ref_overlap_ema"]) < 12.0:
            policy["frame0_hard_scale"] = max(float(policy.get("frame0_hard_scale", 1.0)), 0.65)
            policy["reference_hard_scale"] = max(float(policy.get("reference_hard_scale", 1.0)), 0.70)
    ...
    def _update_geo_structure_ready_streak(self, frame_idx: int) -> bool:
        ...
        if raw_ready:
            ...
            if mature_structure:
                strong_unready = bool(
                    float(self.geo_ref_overlap_ema) < 2.0
                    and float(self.geo_selector_visible_ratio_ema) < 0.55
                )
                if strong_unready:
                    self.geo_structure_ready_streak = 0
    ...
    def _select_geo_active_indices(
        ...
        elif mode_now == "recovery":
            prune_start_ratio_eff = min(prune_start_ratio_eff, 0.62)
        elif mode_now == "reloc":
            prune_start_ratio_eff = min(prune_start_ratio_eff, 0.58)
        if len(self.geo_reference_bank) >= 8 or len(self.geo_landmark_voxels) >= 64:
            prune_start_ratio_eff = min(prune_start_ratio_eff, 0.65)
        if bool((policy or {}).get("allow_reference_refresh_only", False)):
            prune_start_ratio_eff = min(prune_start_ratio_eff, 0.60)
                        ...
                        if use_geo_kv_prune:
                            ...
                            if not anchor_enabled:
                                ...
                                soft_label_ready_now = bool(
                                    structure_ready_now
                                    or self._geo_soft_current_ready(int(past_frame_idx))
                                    or len(self.geo_reference_bank) >= 8
                                    or len(self.geo_landmark_voxels) >= 64
                                    or int(self.geo_reloc_frames_left) > 0
                                    or int(self.geo_recovery_frames_left) > 0
                                )
