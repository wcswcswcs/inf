...
class PointCloudViewer:
    def __init__(
        ...
        @self.show_gt_checkbox.on_update
        def _(_) -> None:
    ...
    def add_gt_camera(self, step):
        if self.gt_poses is None:
            print(f"gt_poses is None at step {step}")
            return

        if step >= len(self.gt_poses):
            return

        pose = self.gt_poses[step] # (4, 4)
        
        R = pose[:3, :3]
        t = pose[:3, 3]
        const_rot = np.array([[-1, 0, 0], [0, 1, 0], [0, 0, -1]]) 
        R = R @ const_rot
        q = tf.SO3.from_matrix(R).wxyz
        ...
        if hasattr(self, 'show_gt_checkbox'):
            handle.visible = self.show_gt_checkbox.value
